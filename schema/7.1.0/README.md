...
* [`AnnData.obs`](https://github.com/scFAIR/scFAIR/blob/main/schema/7.1.0/schema.md#obs-cell-metadata) - Cell metadata. Describe each cell in the dataset.
  ...
  * [`tissue_ontology_term_id`](https://github.com/scFAIR/scFAIR/blob/main/schema/7.1.0/schema.md#tissue_ontology_term_id) [`str`] - UBERON term(s) if `tissue_type` is `"tissue"` or `"organoid"`. Cellosaurus term(s) if `tissue_type` is `"cell line"`, or CL term(s) if `tissue_type` is `"primary cell culture"`. Can be multiple terms in ascending lexical order separated by the delimiter `" || "` with no duplication of identifiers.
  ...
  * [`cell_type_ontology_term_id`](https://github.com/scFAIR/scFAIR/blob/main/schema/7.1.0/schema.md#cell_type_ontology_term_id) [`str`] - CL term(s) (or `"unknown"`) describing the curated cell-type. e.g. `"CL:0000115"` for *endothelial cell*. Only present if `uns['is_pre_analysis'] = False`. Can be cross-referenced species-specific ontology term(s) such as `"FBbt:00046052"` for *fat cell* in flies. Can be multiple terms in ascending lexical order separated by the delimiter `" || "` with no duplication of identifiers.
  * *[`cell_type`](https://github.com/scFAIR/scFAIR/blob/main/schema/7.1.0/schema.md#cell_type)* [`str`] (Paired with `cell_type_ontology_term_id`) - Human-readable name(s) assigned to the value of `cell_type_ontology_term_id`.
  * [`development_stage_ontology_term_id`](https://github.com/scFAIR/scFAIR/blob/main/schema/7.1.0/schema.md#development_stage_ontology_term_id) [`str`] - UBERON term(s) (or `"unknown"`, or `"na"` if `tissue_type` is `"cell line"`) describing the curated developmental stage. e.g. `"UBERON:0000068"` for *embryo stage*. Can be UBERON cross-referenced species-specific ontology term(s) such as `"FBdv:00007077"` for *day 2 of adulthood* in flies. Can be multiple terms in ascending lexical order separated by the delimiter `" || "` with no duplication of identifiers.
  * *[`development_stage`](https://github.com/scFAIR/scFAIR/blob/main/schema/7.1.0/schema.md#development_stage)* [`str`] (Paired with `development_stage_ontology_term_id`) - Human-readable name(s) assigned to the value of `development_stage_ontology_term_id`.
  ...
  * [`self_reported_ethnicity_ontology_term_id`](https://github.com/scFAIR/scFAIR/blob/main/schema/7.1.0/schema.md#self_reported_ethnicity_ontology_term_id) [`str`] - HANCESTRO term(s) (or "unknown") only if organism is human ("na" for other organisms or if `tissue_type` is `"cell line"`). Can be multiple terms in ascending lexical order separated by the delimiter `" || "` with no duplication of identifiers.
